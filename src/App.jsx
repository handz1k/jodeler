...
import './components/JodelForm'
import './App.css'
import jodelService from './services/jodel.js'
import JodelForm from './components/JodelForm.jsx'
import Jodel from './components/Jodels.jsx'
...
function App() {
  ...
  const jodelFormSubmission = (event) => {
    ...
    setIsLoading(true)
    jodelService.create(jodelObject)
      .then(returnedJodel => {
  ...
  return (
    <div>
      ...
      <div className="jodel-form">
        <main>
          <JodelForm formSubmission={jodelFormSubmission} formValue={newJodel} formHandler={handleJodelChange}/>
      </main>
      </div>
      )}
      {isLoading && (
